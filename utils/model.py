...
class myNN(eqx.Module):
  gcn_layers: list 
  linear_layer:list
  def __init__(self, in_size, hid_size, node_num, out_size=2, graph = True):
    ...
    self.gcn_layers = [
                        MultiHeadGAT(n_heads = 5, in_size=in_size, out_size=hid_size, key = jax.random.PRNGKey(SEED)), 
                        MultiHeadGAT(n_heads = 5, in_size=hid_size, out_size=out_size, key = jax.random.PRNGKey(SEED))
                      ]
    
    self.graph_class = graph
    if self.graph_class:
      self.linear_layer = [ Linear(out_size, hid_size, key=jax.random.PRNGKey(42)),
                           Linear(hid_size, hid_size, key=jax.random.PRNGKey(42)),
                           Linear(hid_size, out_size, key=jax.random.PRNGKey(42))
      ]
    else:
      self.linear_layer = []
  ...
  def __call__(self, x, adj):
    for layer in self.gcn_layers:
      x = jax.nn.relu(layer(x, adj, jax.random.PRNGKey(SEED)) )
      x = jnp.mean(x, axis = 0).reshape([-1, 1])
      print("mean pool shape", x.shape)
      # ### pooling here
      for layer in self.linear_layer:
        x = jax.nn.relu(layer(x))
    # print("The out put size", x.shape)
